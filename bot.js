...
console.log('Failed To Join: n The Channel Type isn "Listening."')
}
});
//BY 5AO
//BY 5AO
...
//:speaker: Volume changed from 20 to 20 ! The volume has been changed from ${queue.volume} to ${args[0]}
      ...
      for (var i = 0; i < queue.songs.length; i++) {
        ...
        text += `${num} ${queue.songs[i].title} [${queue.songs[i].duration}]n`
      }
      embed.setDescription(`Songs Queue | ${msg.guild.name}nn ${text}`)
    ...
    if(queue.repeating) return msg.channel.send('You can't skip, because repeating mode is on, run ' + ``${prefix}repeat` to turn off.`);
      ...
      if(!queue || !queue.songs[0]){
        ...
        eb.setTitle("No music playing");
            eb.setDescription("u23F9 "+bar(-1)+" "+volumeIcon(!queue?100:queue.volume));
      } else if(queue.songs) {
      ...
      function embedFormat(queue) {
        ...
        if(!queue || !queue.songs) {
          return "No music playingnu23F9 "+bar(-1)+" "+volumeIcon(100);
        } else if(!queue.playing) {
          return "No music playingnu23F9 "+bar(-1)+" "+volumeIcon(queue.volume);
        } else {

          let progress = (queue.connection.dispatcher.time / queue.songs[0].msDur);
          let prog = bar(progress);
          let volIcon = volumeIcon(queue.volume);
          let playIcon = (queue.connection.dispatcher.paused ? "u23F8" : "u25B6")
...
**`
  ...
  if(message.content === prefix + 'help') {
    ...
    let e = '** جاري الارســال .. :envelope_with_arrow: **'
	  ...
	  message.author.send(helplist).catch(error => message.reply('** لم اتمكن من الارسال الاوامر لك ,شيك خاصك اذا مفتوح لاهنت :negative_squared_cross_mark:**'))
}
});
